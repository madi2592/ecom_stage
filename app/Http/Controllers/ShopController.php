...
class ShopController extends Controller
{
    ...
    public function catalogue(Request $request)
    {
        ...
        //    ✅ 'imagePrincipale' ajoutée — nécessaire pour ProductCard
        $query = Produit::with(['images', 'imagePrincipale', 'categorie', 'variantes'])
            ->where('est_actif', true);
        ...
        // 6. Données pour la Sidebar
        $categories = Categorie::withCount(['produits' => function ($q) {
            $q->where('est_actif', true);
        }])->get();

        $availableColors = VarianteProduit::distinct()
            ->whereNotNull('couleur')
            ->pluck('couleur');

        return Inertia::render('Shop/Catalogue', [
            // ✅ withQueryString() préserve les paramètres de filtre dans les liens de pagination
