...
export default function FrontLayout({ children }) {
    //  Récupère les props globales
    const { auth, panier, cartCount } = usePage().props;
    ...
    //Quantité de panier
    const total = Object.values(panier || {}).reduce((acc, item) => acc + (item.prix * item.quantite), 0);
    ...
    const removeItem = (cartKey) => {
        router.delete(route('panier.supprimer', cartKey), { preserveScroll: true });
    };
...
<div className={`fixed top-0 right-0 h-full w-full sm:w-[450px] bg-white z-[70] shadow-2xl transform transition-transform duration-500 ${isCartOpen ? "translate-x-0" : "translate-x-full"}`}>
    <div className="p-8 flex flex-col h-full">
        ...
        {/* Corps du panier (Dynamique) */}
        <div className="flex-1 overflow-y-auto">
            {Object.values(panier).length > 0 ? (
                <div className="space-y-6">
                    {Object.values(panier).map((item) => (
                        <div key={item.cart_key} className="flex gap-4 border-b border-gray-100 pb-6">
                            <div className="w-20 h-24 bg-gray-100 rounded-lg overflow-hidden shrink-0">
                                <img src={`/storage/${item.image}`} alt={item.libelle} className="w-full h-full object-cover" />
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between items-start">
                                    <h3 className="text-xs font-bold uppercase tracking-tight text-black">{item.libelle}</h3>
                                    <button onClick={() => removeItem(item.cart_key)} className="text-gray-300 hover:text-red-500">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                                <p className="text-[10px] text-gray-400 uppercase mt-1 italic">{item.taille} / {item.couleur}</p>
                                <div className="flex justify-between items-center mt-4">
                                    <div className="flex items-center border border-gray-200 rounded">
                                        <button onClick={() => updateQty(item.cart_key, 'moins')} className="p-1 hover:bg-gray-50"><Minus size={12} /></button>
                                        <span className="px-3 text-xs font-bold text-black">{item.quantite}</span>
                                        <button onClick={() => updateQty(item.cart_key, 'plus')} className="p-1 hover:bg-gray-50"><Plus size={12} /></button>
                                    </div>
                                    <span className="font-bold text-sm text-black">{item.prix} DH</span>
                                </div>
                            </div>
                        </div>
                    ))}
                ...
                <div className="h-full flex flex-col items-center justify-center text-center py-20 italic text-gray-400">
        ...
        <div className="border-t pt-6">
            ...
            <div className="flex flex-col gap-3">
                <Link 
                    ...
                    className="w-full border-2 border-black text-black py-4 font-bold uppercase tracking-widest text-center text-xs hover:bg-black hover:text-white transition-all"
                >
                    Voir le panier complet
                </Link>
                <button 
                    disabled={Object.values(panier).length === 0}
                    className="w-full bg-[#0315ff] text-white py-4 font-bold uppercase tracking-widest hover:bg-black transition-colors disabled:bg-gray-200 disabled:cursor-not-allowed"
                >
                    Commander
                </button>
            </div>
            ...
            <div
                className={`fixed top-0 left-0 h-full w-[300px] bg-white z-[70] shadow-2xl transform transition-transform duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
            >
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex flex-col px-6 py-6 gap-0 overflow-y-auto flex-1">
                        ...
                        <div className="border-b border-gray-100">
                            ...
                            <div
                                className={`overflow-hidden transition-all duration-300 ${isMobileServiceOpen ? "max-h-40 pb-3" : "max-h-0"}`}
                            >
                                <Link
                                    href="#"
                                    onClick={() => setIsMenuOpen(false)}
            ...
            <header className="fixed top-0 left-0 right-0 w-full h-[85px] bg-white border-b border-gray-100 flex items-center z-50">
                <div className="flex w-full h-full">
                    ...
                    <div className="w-[180px] sm:w-[280px] flex items-center justify-center border-r border-gray-100 shrink-0">
                        <Link
                            href="/"
                            className="text-2xl sm:text-3xl font-black tracking-tighter text-black"
                        >
                            Click'n'<span className="text-[#0315ff]">Buy</span>
                        </Link>
                    </div>
                    ...
                    <nav className="hidden lg:flex flex-1 items-center px-10 gap-8 uppercase text-xs font-bold tracking-widest text-black">
                        <Link
                            href="/"
                            className="hover:text-[#0315ff] transition-colors"
                        >
                            Accueil
                        </Link>
                        <Link
                        ...
                        <div className="relative" ref={dropdownRef}>
                            ...
                            <div
                                className={`absolute top-full left-0 mt-4 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl overflow-hidden transition-all duration-200 origin-top ${isDropdownOpen ? "opacity-100 scale-y-100 pointer-events-auto" : "opacity-0 scale-y-95 pointer-events-none"}`}
                            >
                                <Link
                                    href="#"
...
>
    <ShoppingBag size={22} />
    {cartCount > 0 && ( // On affiche le badge seulement s'il y a des articles
        <span className="absolute -top-2 -right-2 bg-[#0315ff] text-white text-[10px] w-4 h-4 rounded-full flex items-center justify-center font-bold">
            {cartCount}
        </span>
    )}
</button>
                ...
                <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12">
                    <div className="col-span-1">
                        <div className="text-3xl font-black text-white mb-6">
                            Click'n'<span className="text-[#2834e3]">Buy</span>
                        </div>
                        <p className="text-gray-500 text-sm">
                            Le meilleur de la mode au Maroc.
                        </p>
                    </div>
                    <div>
